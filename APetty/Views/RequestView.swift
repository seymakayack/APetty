            ...
            if let document = document, document.exists {
                ...
                db.collection("requests").addDocument(data: requestData) { error in
                    if let error = error {
                        ...
                        fetchRequests()  // Ekleme sonrası talepleri yeniden yükle
                    }
                }
            } else {
    ...
    func fetchRequests() {
        db.collection("requests").order(by: "timestamp").getDocuments { snapshot, error in
            if let error = error {
                ...
                print("Fetched requests: \(requests)")  // Verileri kontrol et
            }
        }
    }
}
